...
private:
   ...
   //returns the node that we want to insert at 
   TreeNode* traverseToNode(TreeNode*, string);
   
   //helper function to find a node
   DBentry* findHelper(TreeNode*, string);
   
   //returns the node above the node we are at
   TreeNode* traverseToNodeAbove(TreeNode*, string);
   
   //finds the node that has the largest subtree and disconnects it
   TreeNode* removeLargestSubtree(TreeNode*);
   
   //recursively move down the tree to delete all nodes
   void removeAll(TreeNode*);
   
   //recursively traverse the entire tree
   int traverseBT(TreeNode*) const;
   
   //recursively traverse the entire tree and print each node
